...
import br.pub.cashfinder.control.Executer;
...
public class FindActionListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			fillEntry();
			Executer exec = new Executer(this.frame.getProgress());
			exec.start();
	...
	private void fillEntry() throws IOException {
		...
		for (int i = 0; i < find; i++) {
			this.writer.write(this.frame.getFindList().get(i).toString().replace(".", ",") + "\n");
		}
		for (int i = 0; i < aviso; i++) {
			this.writer.write(this.frame.getAvisoList().get(i).toString().replace(".", ",") + "\n");
		}
		this.writer.close();
	}
